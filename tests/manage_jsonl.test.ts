...
describe("manage_jsonl CLI", () => {
	...
	it("validates optional interview graph artifacts", () => {
		const root = tempDir();
		const topicDir = path.join(root, ".plan", "demo");
		fs.mkdirSync(topicDir, { recursive: true });
		fs.writeFileSync(path.join(root, "README.md"), "hello\n", "utf8");
		...
		const report = runJson(["validate-topic", "--root", root, "--topic", "demo"]);
		expect(report.ok).toBe(true);
		expect(report.counts.interview_nodes).toBe(2);
		expect(report.counts.interview_edges).toBe(2);
		...
		expect(errors).toContain("Invalid interview status");
		expect(errors).toContain("references missing requirement");
		expect(errors).toContain("references missing design");
		expect(errors).toContain("references missing fact");
		expect(errors).toContain("must use interview.md#heading");
		expect(errors).toContain("Invalid interview edge type");
		expect(errors).toContain("Direct private artifact reference");
	});
